...
<script lang="ts">
definePageMeta({
  alias: "/productos/cuentas/:categoryID/item/:productID",
  seoManual: true,
});
...
@NuxtComponent({
  name: "products-accounts-item-screen",
  components: {
  ...
  async setup() {
    const vm = getCurrentInstance()!.proxy as ProductsAccountsItemScreen;
...
export default class ProductsAccountsItemScreen extends Vue {
  ///////////////
  // VARIABLES //
  ///////////////
  ...
  public setMenuOptions() {
    this.menuOptions = {
      route: "/productos/cuentas/" + this.pageParam + "/item/",
      current: this.pageParam,
      default: {
        name: this.category.name,
        icon: "mdi-card",
        to: "/productos/cuentas/" + this.pageParam,
      },
      items: this.products,
    };
  }
