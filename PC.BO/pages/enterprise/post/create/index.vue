...
// IMPORT MUTATIONS
import { CREATE_ENTERPRISE } from "~/graphql/mutations/enterprise.mutation";
...
class EnterpriseCreateScreen extends Vue {
  ///////////////
  ///// REF /////
  ///////////////
  ...
  public async createEnterprise() {
    ...
    try {
      ...
      createEnterpriseDto.createEnterpriseDto.sections = this.enterprise.sections.map((section) => {
  ...
  // CREATE ENTERPRISE CARD
  public async createEnterpriseDraft() {
    // SET LOADING FALSE
    this.loading = true
  ...
  public get validCreatePost(): boolean {
    return this.enterprise.title === '' ||
      this.enterprise.excerpt === '' ||
      this.enterprise.subtitle === '' ||
      this.enterprise.description === '' ||
      !String(this.enterprise.bannerImageDetail?.image ?? '').trim()
  }

  public get invalidImages(): boolean {
    return (
      !String(this.enterprise.bannerImageDetail?.image ?? '').trim() || 
      !String(this.enterprise.thumbnailImageDetail?.image ?? '').trim() || 
      !String(this.enterprise.responsiveImageDetail?.image ?? '').trim()
    );
  }
  ...
  public async goPreview() {
    ...
    const post = await this.createEnterpriseDraft();

    // GO EDIT AFTER CREATE POST
    this.$router.push(`/enterprise/post/update/${post.createEnterprise._id}`);

    // GO PREVIEW BEFORE CREATE PRODUCT
    const route = this.$router.resolve({ 
      path: '/previews/post',
      query: {
        postID: post.createEnterprise._id, 
        postType: 'enterprise'
      } 
    });
...
export default EnterpriseCreateScreen;
</script>
...
<template>
  <v-row class="px-0 pt-10" justify="center" align-content="center">
    ...
    <v-bottom-navigation bg-color="#12539b">
      <v-btn @click="createEnterprise()" :disabled="validCreatePost || invalidImages" :loading="loading">
        <v-icon>
          mdi-upload
        </v-icon>
        Crear Post
      </v-btn>
      <v-btn @click="saveEnterpriseDraft()" :disabled="validCreatePost" :loading="loading">
        <v-icon>
          mdi-content-save
        </v-icon>
        Guardar
