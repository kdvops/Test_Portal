...
@NuxtComponent({
  name: "products-categories-update-screen",
  components: {
    "category-form-page": CategoryFormPage,
  },
})
class ProductsCategoriesUpdateScreen extends Vue {
  // Route params
  ...
  public async created() {
    ...
    this.targetManager.selectTarget("categoryProducts");
  ...
  public async setTargetInfo() {
    try {
      ...
      this.targetInfo = {
        _id: "categoryProducts",
        name: "Products",
        color: "#12539b",
        description: "Categorías de Products",
        status: "publish",
        createdAt: new Date(),
        updatedAt: new Date(),
        slug: "",
  ...
  public async goPreview() {
    ...
    const route = this.$router.resolve({
      path: `/previews/category/${"products"}`,
    });
  ...
  public goUpdate(categoryID: string) {
    this.$router.push(`/products/categories/update/${categoryID}`);
  }
}
export default ProductsCategoriesUpdateScreen;
</script>
...
<template>
  <category-form-page
    ...
    :target-name="targetInfo?.name || 'Products'"
    :loading="loading"
