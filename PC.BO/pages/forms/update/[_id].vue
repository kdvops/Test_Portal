...
// IMPORT MUTATIONS
import { UPDATE_FORM_BSC } from "~/graphql/mutations/forms.mutation";
...
@NuxtComponent({
  name: "forms-update-screen",
  components: {
    "detailed-image-component": DetailedImageComponent,
  },
})
class FormsUpdateScreen extends Vue {
  ///////////////
  ///// REF /////
  ///////////////
  ...
  public async setFormByID() {
    try {
      ...
      // FORM DATA VALUES
      const inputs = [...data.findFormsById.inputs];
      const form = { ...data.findFormsById };

      if (form.crm && form.crm.isFormCrm) {
        // CRM VALUES
        this.isFormCrm = form.crm.isFormCrm;
        this.crmUrl = form.crm.url;
      }

      // SET NEW OBJECT FORM
      Object.assign(this.formBsc, { ...form, inputs });
    } catch (err) {
      // SHOW ERROR
  ...
  public async updateForm() {
    ...
    this.loading = true;
    try {
      const formBscClean = _.omit(this.formBsc, [
        "_id",
        "__typename",
        "termsAndCondition.__typename",
        "crm.__typename",
      ]);

      // OMIT FIELDS NOT NECESSARY IN SECTION
      const formInputsClean = formBscClean.inputs!.map((input: any) => {
        let cleanInput = _.omit(input, [
          "__typename",
          "radios.__typename",
          "selects.__typename",
          "checkbox.__typename",
      ...
      // PAYLOAD LOGIN DTO
      const updateFormsDto = {
        updateFormsDto: {
          formsID: this.formBsc._id,
          forms: {
            ...formBscClean,
            crm: {
              isFormCrm: this.isFormCrm,
              url: this.crmUrl,
            },
            status: "publish",
            inputs: formInputsClean,
          },
      ...
      await this.$apollo.mutate({
        mutation: UPDATE_FORM_BSC,
        variables: updateFormsDto,
      });
  ...
  public removeInput(index: number) {
    const stateInputForm = [...this.formBsc.inputs];
    stateInputForm.splice(index, 1);

    this.formBsc = {
      ...this.formBsc,
      inputs: [...stateInputForm],
    };
  }
  ...
  public updateInput() {
    ...
    const stateInputForm = [...this.formBsc.inputs];
    const newInputForm = { ...stateInputForm[position], ...this.dialog.input };

    stateInputForm[position] = newInputForm;
  ...
  public async goPreview() {
    ...
    const route = this.$router.resolve({
      path: `/previews/form/${this.formID}`,
    });
...
export default FormsUpdateScreen;
</script>
  ...
  <v-row class="px-5 pt-10" justify="center" align-content="center">
    ...
    <v-bottom-navigation bg-color="#12539b">
      <v-btn
        ...
        rounded="xl"
        :base-color="
          formBsc.status && formBsc.status === 'draft' ? 'orange' : 'green'
        "
      ...
      >
        {{
          formBsc.status && formBsc.status === "draft"
            ? "Borrador"
            : "Publicado"
        }}
      </v-btn>
