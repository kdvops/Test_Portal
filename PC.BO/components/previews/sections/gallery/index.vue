...
class SectionGalleryComponent extends Vue {
  ...
  public mounted() {
    ...
    const exists = (tag: 'script' | 'link', attr: 'src' | 'href', value: string) =>
      !!document.querySelector(`${tag}[${attr}="${value}"]`);

    const cssHref = "https://cdn.jsdelivr.net/npm/uikit@3.16.26/dist/css/uikit.min.css";
    ...
    const jsSrc1 = "https://cdn.jsdelivr.net/npm/uikit@3.16.26/dist/js/uikit.min.js";
    ...
    const jsSrc2 = "https://cdn.jsdelivr.net/npm/uikit@3.16.26/dist/js/uikit-icons.min.js";
            ...
            <ul :class="`uk-slider-items uk-child-width-1-${(section.gallery?.items.length || 0) < 6? section.gallery?.items.length:5}@s uk-child-width-1-${(section.gallery?.items.length || 0) < 6? section.gallery?.items.length:5}@`">
              <li v-for="item in section.gallery?.items" @click="openDialog(item)">
                <div class="multivideos-slider-background-container uk-flex uk-flex-center uk-background-cover"
                  :data-src="item.image ?? item.imageDetail?.image" uk-img="loading: eager">
                  <div uk-grid class="multivideos-slider-info-container">
                    ...
                    <div v-if="item.icon || item.title" class="multivideos-slider-title-container">
                      <img 
                        v-if="item.icon"
                        :src="item.icon ?? item.iconImageDetail?.image"
    ...
    <v-card width="100%" height="100%" rounded="xl">
      <v-card-title class="text-h6 font-weight-bold text-white py-5" style="background-color: var(--bsc-primary-color);">
        ...
        <v-btn class="mt-5 mr-5" color="red" density="compact" position="absolute" location="end top"
          @click="closeDialog()" icon>
          <v-icon size="16" color="white">mdi-close</v-icon>
        </v-btn>
      </v-card-title>
      <v-card-text class="pa-0">
        <iframe width="100%" height="600" :src="`https://www.youtube.com/embed/${dialog.video.video}`"
          frameborder="0"
...
.multivideos-slider-background-container:hover .multivideos-slider-btn-container {
  opacity: 1;
}
