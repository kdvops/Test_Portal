...
@NuxtComponent({
  name: "enterprise-categories-update-screen",
  components: {
    "category-form-page": CategoryFormPage,
  },
})
class EnterpriseCategoriesUpdateScreen extends Vue {
  // Route params
  ...
  public async created() {
    ...
    this.targetManager.selectTarget("categoryEnterprise");
  ...
  public async setTargetInfo() {
    try {
      ...
      this.targetInfo = {
        _id: "categoryEnterprise",
        name: "Enterprise",
        color: "#12539b",
        description: "Categorías de Enterprise",
        status: "publish",
        createdAt: new Date(),
        updatedAt: new Date(),
        slug: "",
  ...
  public async updateCategory() {
    ...
    try {
      ...
      this.$router.push(`/enterprise/categories/list`);
  ...
  public async goPreview() {
    ...
    const route = this.$router.resolve({
      path: `/previews/category/${'enterprise'}`,
    });
  ...
  public goUpdate(categoryID: string) {
    this.$router.push(
      `/enterprise/categories/update/${categoryID}`
    );
  }
}
export default EnterpriseCategoriesUpdateScreen;
</script>
...
<template>
  <category-form-page
    ...
    :target-name="targetInfo?.name || 'Enterprise'"
    :loading="loading"
