...
import { GET_PRODUCT_ADJUDICATED_BY_ID } from "~/graphql/query/adjudicated.query";

// IMPORT MUTATIONS
import { UPDATE_PRODUCT_ADJUDICATED } from "~/graphql/mutations/adjudicated.mutation";
...
class AdjudicatedProductUpdateScreen extends Vue {
  ///////////////
  ///// REF /////
  ///////////////
  ...
  public created() {
    // SET CATEGORIES ADJUDICATED
    this.setCategories();

    // SET PRODUCT ADJUDICATED
    this.setProductAdjudicated();
  ...
  public removeImage(index: number) {
    const getImage = this.product.pictures[index];
    if (getImage.newImage && getImage.newImage.length > 0 && getImage.newImage[0].img) {
      this.product.pictures.splice(index, 1);
    } else {
      this.product.pictures[index].action = 'delete'
    }
  }
  ...
  // SET PRODUCT ADJUDICATED
  public async setProductAdjudicated() {
    try {
      // GET CATEGORY
      const { data } = await this.$apollo.query({
        query: GET_PRODUCT_ADJUDICATED_BY_ID,
        variables: {
          adjudicatedId: this.productID,
        },
      ...
      this.product = {
        ...data.findAdjudicatedById,
        description: data.findAdjudicatedById.description
          ? decrypt(data.findAdjudicatedById.description)
          : "",
      }

      // SET HTML EDITOR
      setTimeout(() => {
        this.descriptionEditor?.setHTML(this.product.description);
      }, 200);

      // SET PARENT CATEGORY SELECTED
      this.setCategorySelected([data.findAdjudicatedById.category]);
  ...
  // UPDATE PRODUCT ADJUDICATED
  public async updateAdjudicatedProduct() {
    // SET LOADING FALSE
    this.loading = true;
    try {

      // OMIT VALUES
      const productClean = _.omit(this.product, ['_id', '__typename'])

      // OMIT VALUES IN PICTURES
      const picturesClean = this.product.pictures.map((picture: any) => _.omit(picture, ['__typename']))

      // PAYLOAD PARENT AND TARGET DTO
      const updateAdjudicatedDto = {
        updateAdjudicatedDto: {
          adjudicatedID: this.product._id,
          adjudicated: {
            ...productClean,
            item_status: 'publish',
            pictures: picturesClean,
            price: Number(productClean.price),
      ...
      await this.$apollo.mutate({
        mutation: UPDATE_PRODUCT_ADJUDICATED,
        variables: updateAdjudicatedDto,
        fetchPolicy: "no-cache",
      });
  ...
  // UPDATE PRODUCT ADJUDICATED
  public async updateAdjudicatedProductDraft() {
    // SET LOADING FALSE
    this.loading = true;
    try {
   ...
   // SAVE ITEM
   public async saveAdjudicatedProductDraft() {
    // FIRST UPDATE PRODUCT STATUS DRAFT
    await this.updateAdjudicatedProductDraft();
  ...
  public async goPreview() {
    ...
    // GO PREVIEW CATEGORY
    const route = this.$router.resolve({ path: `/previews/category/adjudicated/${this.product.category}` });
  ...
  get picturesImageDetail(){
    return this.product.picturesImageDetail?? []
  }

  set picturesImageDetail(newValue){
    this.product.picturesImageDetail = newValue
  }

  get picturesImageDetailed(){
    this.product.picturesImageDetail = this.picturesImageDetail.concat(this.product.pictures? this.product.pictures.filter((picture) => picture.image).map((picture) => ({
        _id: picture._id,
        image: picture.image,
        altText: null,
  ...
  set picturesImageDetailed(newValue){
    this.product.picturesImageDetail = newValue
  }
...
export default AdjudicatedProductUpdateScreen;
</script>
  ...
  <v-row class="px-0 pt-10" justify="center" align-content="center">
        ...
        <v-row>
          ...
          <v-col cols="4">
            <v-select v-model="product.status" :items="statusList" item-title="name" item-value="value"
              density="compact" variant="solo" rounded="xl" label="Estado del producto">
                  ...
                  <v-img height="180" :src="picture.image?? ''" contain>
                    <v-menu>
                      <template v-slot:activator="{ props }">
                        <v-btn icon="mdi-dots-vertical" :variant="isHovering? 'flat':'text'" v-bind="{...props}"></v-btn>
                      </template>
                      <v-list>
    ...
    <v-bottom-navigation bg-color="#12539b">
      <v-btn position="absolute" rounded="xl"
        :base-color="product.item_status && product.item_status === 'draft' ? 'orange' : 'green'"
        style="height: 30px; top: 15px; left: 10px" variant="tonal" readonly>
        {{ product.item_status && product.item_status === 'draft' ? 'Borrador' : 'Publicado' }}
      </v-btn>
      <v-btn @click="updateAdjudicatedProduct()" :disabled="!valid || !product.category || product.picturesImageDetail.length === 0" :loading="loading">
        <v-icon>
          mdi-upload
        </v-icon>
        Actualizar y publicar
      </v-btn>
      <v-btn @click="saveAdjudicatedProductDraft()" :disabled="!valid || !product.category || product.picturesImageDetail.length === 0" :loading="loading">
        <v-icon>
          mdi-content-save
        </v-icon>
        Guardar borrador
