...
@NuxtComponent({
  name: "products-loans-update-screen",
  components: {
...
class ProductsLoansUpdateScreen extends Vue {
  ///////////////
  // VARIABLES //
  ///////////////
  ...
  // UPDATE PRODUCT CARD
  public async updateProductLoans() {
    // SET LOADING
    this.loading = true;
...
export default ProductsLoansUpdateScreen;
</script>
...
<template>
  <v-row class="px-0 pt-10" justify="center" align-content="center">
    ...
    <v-bottom-navigation bg-color="#12539b">
      ...
      <v-btn @click="updateProductLoans()" :disabled="!valid || product.sections.length === 0" :loading="loading">
        <v-icon>
          mdi-upload
        </v-icon>
