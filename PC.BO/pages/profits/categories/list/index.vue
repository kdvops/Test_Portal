...
@NuxtComponent({
  name: "profits-categories-list-screen",
  components: {
...
class ProfitsCategoriesListScreen extends Vue {
  ///////////////
  // VARIABLES //
  ///////////////
  ...
  public async setCategories() {
    try {
      await this.categoryStore.loadCategories("category::profits");
  ...
  // GO TO CREATE SLIDER
  public goCreate() {
    this.$router.push(`/profits/categories/create`);
  }

  // GET CATEGORIES FOR TARGET
  public get categoriesForTarget(): Array<CategoriesInterface> {
    const categories = this.categories;
    const categoriesProfits = categories.filter(
      (category: CategoriesInterface) =>
        category.target === "categories::profits"
    );

    return categoriesProfits;
  }
...
export default ProfitsCategoriesListScreen;
</script>
          ...
          <v-card-text>
            <v-row align-content="center" justify="space-between">
              ...
              >
                ...
                >
                  Lista de categorías de beneficios,
                </p>
                <p class="text-caption text-white font-weight-light ml-10">
                  Ahora puedes gestionar tus categorías de beneficios de una
                  manera mas sencilla!
                </p>
                <v-btn
                  @click="goCreate()"
