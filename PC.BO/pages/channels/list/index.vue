...
import AnimationChannels from "~/assets/animations/channels-animation.json";
import {
  CLONE_CHANNEL,
  REMOVE_CHANNEL,
  PUBLISH_CHANNEL,
  DRAFT_CHANNEL,
} from "~/graphql/mutations/channels.mutation";

// IMPORT QUERY'S
import { GET_CHANNELS_POST } from "~/graphql/query/channels.query";
...
class ChannelsListScreen extends Vue {
  ///////////////
  // VARIABLES //
  ///////////////
  ...
  // ANIMATIONS LOTTIE
  public animationChannels: any = AnimationChannels;
  ...
  public async mounted() {
    this.generalStatusComposable.init(
      {
        CLONE_MUTATION: CLONE_CHANNEL,
        REMOVE_MUTATION: REMOVE_CHANNEL,
        PUBLISH_MUTATION: PUBLISH_CHANNEL,
        DRAFT_MUTATION: DRAFT_CHANNEL,
      },
      this.$apollo,
      this.$bus,
      this.$router
  ...
  // SET CHANNELS BY TYPE POST
  public async setChannelsByTypePost() {
    try {
      // GET ALL CATEGORIES
      ...
      // SET CHANNELS
      this.channels = data.findChannelsPost;
    } catch (err) {
      // SHOW ERROR
  ...
  public goUpdate(postID: string) {
    this.$router.push(`/channels/update/${postID}`);
  }
}
            ...
            <v-row align-content="center" justify="space-between">
              ...
              >
                ...
                <p class="text-caption text-white font-weight-light ml-10">
                  Ahora puedes gestionar tus canales de una manera mas sencilla!
                </p>
                <v-btn
                  @click="goCreate()"
                  width="130"
              ...
              <v-col cols="5" class="card-principal-animation">
                <app-lottie
                  ...
                  height="350px"
                  :loop="true"
                  :animationData="animationChannels"
                />
              </v-col>
            </v-row>
          </v-card-text>
  ...
  <v-row class="pa-12">
    <v-col cols="12">
      <v-slide-group class="pa-4" show-arrows>
        <v-slide-group-item v-for="channels in channels" :key="channels._id">
          <v-card width="250" height="300" rounded="xl" class="pa-0 mx-4">
            <v-card-text class="pa-0">
              ...
              >
                <v-col cols="12">
                  <div class="channels-card-image pa-0">
                    <v-img
                      width="100%"
                      height="100%"
                      class="rounded-xl"
                      :src="
                        channels.banner ?? channels.bannerImageDetail?.image
                      "
                      cover
                    >
                      <item-action-component
                        :item="{ ...channels, name: channels.title }"
                ...
                <v-col cols="12" class="channels-card-info pa-3 text-left">
                  <div class="my-2 d-flex align-center">
                    <v-icon
                      class="channels-card-icon"
