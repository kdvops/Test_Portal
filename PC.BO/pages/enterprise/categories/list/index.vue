...
@NuxtComponent({
  name: "enterprise-categories-list-screen",
  components: {
...
class EnterpriseCategoriesListScreen extends Vue {
  // Instances
  ...
  public async setCategories() {
    try {
      await this.categoryStore.loadCategories("category::enterprise");
  ...
  public goUpdate(categoryID: string) {
    this.$router.push(`/enterprise/categories/update/${categoryID}`);
  }

  // GO TO CREATE SLIDER
  public goCreate() {
    this.$router.push(`/enterprise/categories/create`);
  }

  // GET CATEGORIES FOR TARGET
  public get categoriesForTarget(): Array<CategoriesInterface> {
    const categories = this.categories;
    const categoriesEnterprise = categories.filter(
      (category: CategoriesInterface) =>
        category.target === "categories::enterprise"
    );

    return categoriesEnterprise;
  }
...
export default EnterpriseCategoriesListScreen;
</script>
          ...
          <v-card-text>
            <v-row align-content="center" justify="space-between">
              ...
              >
                ...
                >
                  Lista de categorías de Empresa,
                </p>
