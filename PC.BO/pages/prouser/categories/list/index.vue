...
@NuxtComponent({
  name: "prouser-categories-list-screen",
  components: {
...
class ProuserCategoriesListScreen extends Vue {
  ///////////////
  // VARIABLES //
  ///////////////
  ...
  public async setCategories() {
    try {
      await this.categoryStore.loadCategories("category::prouser");
  ...
  public goUpdate(categoryID: string) {
    this.$router.push(`/prouser/categories/update/${categoryID}`);
  }

  // GO TO CREATE SLIDER
  public goCreate() {
    this.$router.push(`/prouser/categories/create`);
  }

  // GET CATEGORIES FOR TARGET
  public get categoriesForTarget(): Array<CategoriesInterface> {
    const categories = this.categories;
    const categoriesProuser = categories.filter(
      (category: CategoriesInterface) =>
        category.target === "categories::prouser"
    );

    return categoriesProuser;
  }
...
export default ProuserCategoriesListScreen;
</script>
          ...
          <v-card-text>
            <v-row align-content="center" justify="space-between">
              ...
              >
                ...
                >
                  Lista de categorías de pro usuario,
                </p>
                <p class="text-caption text-white font-weight-light ml-10">
                  Ahora puedes gestionar tus categorías de pro usuario de una
                  manera mas sencilla!
                </p>
                <v-btn
                  @click="goCreate()"
