...
@NuxtComponent({
  name: "adjudicated-categories-update-screen",
  components: {
    "category-form-page": CategoryFormPage,
  },
})
class AdjudicatedCategoriesUpdateScreen extends Vue {
  // Route params
  ...
  public async created() {
    ...
    // CARGAR TARGETS DINAMICOS Y SELECCIONAR TARGET ACTUAL
    await this.targetManager.loadDynamicTargets(this.$apollo);
    this.targetManager.selectTarget("categoryAdjudicated");
  ...
  public async setTargetInfo() {
    try {
      ...
      this.targetInfo = {
        _id: "categoryAdjudicated",
        name: "Adjudicated",
        color: "#12539b",
        description: "Categorías de Adjudicated",
        status: "publish",
        createdAt: new Date(),
        updatedAt: new Date(),
        slug: "",
  ...
  public async updateCategory() {
    ...
    try {
      ...
      this.$router.push(`/adjudicated/categories/list`);
  ...
  public async goPreview() {
    ...
    const route = this.$router.resolve({
      path: `/previews/category/${"adjudicated"}`,
    });
...
export default AdjudicatedCategoriesUpdateScreen;
</script>
...
<template>
  <category-form-page
    ...
    :target-name="targetInfo?.name || 'Adjudicated'"
    :loading="loading"
      ...
      >
        <v-col cols="12">
          <div class="banner-categories-by-parent pl-2 pr-15">
            <h3 class="text-white text-uppercase">Subcategorias</h3>
          </div>
        </v-col>
