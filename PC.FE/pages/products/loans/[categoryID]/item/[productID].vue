...
<script lang="ts">
definePageMeta({
  alias: "/productos/prestamos/:categoryID/item/:productID",
  seoManual: true,
});
...
@NuxtComponent({
  name: "products-loans-item-screen",
  components: {
  ...
  async setup() {
    const vm = getCurrentInstance()!.proxy as ProductsLoansItemScreen;
...
export default class ProductsLoansItemScreen extends Vue {
  ///////////////
  // VARIABLES //
  ///////////////
  ...
  public setMenuOptions() {
    this.menuOptions = {
      route: "/productos/prestamos/" + this.pageParam + "/item/",
      current: this.pageParam,
      default: {
        name: this.category.name,
        icon: "mdi-card",
        to: "/productos/prestamos/" + this.pageParam,
      },
      items: this.products,
    };
  }
