...
import AnimationCategories from "~/assets/animations/regulatory-animation.json";
...
@NuxtComponent({
  name: "adjudicated-categories-list-screen",
  components: {
...
class AdjudicatedCategoriesListScreen extends Vue {
  ///////////////
  // VARIABLES //
  ///////////////
  ...
  public async setCategories() {
    try {
      await this.categoryStore.loadCategories("category::adjudicated");
  ...
  public goUpdate(categoryID: string) {
    this.$router.push(`/adjudicated/categories/update/${categoryID}`);
  }

  // GO TO CREATE SLIDER
  public goCreate() {
    this.$router.push(`/adjudicated/categories/create`);
  }

  // GET CATEGORIES FOR TARGET
  public get categoriesForTarget(): Array<CategoriesInterface> {
    const categories = this.categories;
    const categoriesAdjudicated = categories.filter(
      (category: CategoriesInterface) =>
        category.target === "categories::adjudicated"
    );

    return categoriesAdjudicated;
  }
...
export default AdjudicatedCategoriesListScreen;
</script>
          ...
          <v-card-text>
            <v-row align-content="center" justify="space-between">
              ...
              >
                ...
                >
                  Lista de categorías de bienes,
                </p>
                <p class="text-caption text-white font-weight-light ml-10">
                  Ahora puedes gestionar tus categorías de bienes adjudicados de
