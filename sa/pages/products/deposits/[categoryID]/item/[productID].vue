...
<script lang="ts">
definePageMeta({
  alias: "/productos/depositos/:categoryID/item/:productID",
  seoManual: true,
});
...
@NuxtComponent({
  name: "products-deposits-item-screen",
  components: {
  ...
  async setup() {
    const vm = getCurrentInstance()!.proxy as ProductsDepositsItemScreen;
...
export default class ProductsDepositsItemScreen extends Vue {
  ///////////////
  // VARIABLES //
  ///////////////
  ...
  public setMenuOptions() {
    this.menuOptions = {
      route: "/productos/depositos/" + this.pageParam + "/item/",
      current: this.pageParam,
      default: {
        name: this.category.name,
        icon: "mdi-card",
        to: "/productos/depositos/" + this.pageParam,
      },
      items: this.products,
    };
  }
