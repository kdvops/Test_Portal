...
import type {
  NewPicturePromotion,
  PromotionInterface,
  DialogCreatePromotionInterface,
...
// MUTATIONS
import { UPDATE_PROMOTIONS } from "~/graphql/mutations/promotion.mutation";
...
class PromotionCreateScreen extends Vue {
  ...
  // GET PARAM SLIDER ID
  public monthParam: any = useRoute().params.month;
  ...
  // SET PROMOTIONS
  public async setPromotionsByMonth() {
    // GET MONTH PARAM
    let month: string = this.monthParam
    ...
    try {
      ...
      // SET PROMOTIONS TO VARIABLE
      this.promotions = data.findPromotionsByMonth[0].promotions;
    } catch (err) {
      // SHOW ERROR
  ...
  public async updatePromotions() {
    ...
    try {
      // CLEAN PROMOTIONS
      const promotions = this.promotions.map(
        (promotion: PromotionInterface) => {
          const omit = _.omit(promotion, ["date.__typename", "__typename"]);
          return { ...omit, status: 'publish' };
        }
      );
      ...
      await this.$apollo.mutate({
        mutation: UPDATE_PROMOTIONS,
        variables: updatePromotionDto,
      });
      ...
      this.$bus.$emit("showSnackbar", {
        text: "Promociones actualizadas correctamente!",
        color: "success",
        timeout: 6000,
      });
    } catch (err) {
  ...
  // UPDATE PROMOTION
  public async updatePromotionsDraft() {
    // SET LOADING TRUE
    this.loading = true;
    try {
      // CLEAN PROMOTIONS
  ...
  // SAVE ITEM
  public async savePromotionsDraft() {
    // FIRST UPDATE PRODUCT STATUS DRAFT
    await this.updatePromotionsDraft();
  ...
  public async goPreview() {
    ...
    const route = this.$router.resolve({
      ...
      query: {
        start: this.$app.$moment(this.monthParam, 'YYYY-MM').startOf('month').format("YYYY-MM-DD"),
        end: this.$app.$moment(this.monthParam, 'YYYY-MM').endOf('month').format("YYYY-MM-DD")
      }
    });
  ...
  <v-row class="pa-5" justify="center">
    ...
    <v-col cols="6">
      <v-alert class="mb-3" rounded="xl" color="info" icon="$info" title="Información" prominent>
        <p class="text-body-2 my-2">
          Puedes gestionar tus promociones de manera continua he ir agregando
          todas las que necesites, <br />
          se le debe colocar a cada promoción su respectiva fecha de apertura y
          cierre
        </p>
      </v-alert>
    </v-col>
    <v-col cols="6">
      <v-alert class="mb-3" rounded="xl" color="warning" icon="$warning" title="Importante" prominent>
        <p class="text-body-2 my-2">
          Las Promociones trabajan de manera independiente, es decir que cada
          una tiene su fecha para ser filtrada <br />
          de manera automática en la vista de promociones BSC, con el fin de
          poder ser reutilizadas en el futuro
        </p>
      </v-alert>
    </v-col>
    ...
    <v-col cols="12" class="pb-15 mb-15">
      <v-row align-content="center" justify="center">
        ...
        <v-col class="banner-create-card py-0 px-5 mt-5 mx-auto text-center align-center d-flex" cols="12">
          <p class="text-weight-bold">Crear nueva tarjeta de promoción</p>
        ...
        <v-col v-for="(promotion, i) in promotions" cols="3">
          <v-card width="250" :key="promotion.name" rounded="xl" class="pa-0">
    ...
    <v-dialog class="dialog-create-promotion" v-model="dialog.show" max-width="500" persistent>
      <v-card class="pa-0" rounded="xl" color="primary">
        <v-card-text v-if="dialog && dialog.promotion">
          <v-row align-content="center" justify="center" no-gutters>
            <v-col cols="12" class="text-center">
              <detailed-image-component color="#00a44f" rounded="xl" height="100"
    ...
    <v-bottom-navigation bg-color="#12539b">
      <v-btn @click="updatePromotions()" :disabled="promotions.length < 1" :loading="loading">
        <v-icon>
          mdi-upload
        </v-icon>
        Actualizar y publicar
      </v-btn>
      <v-btn @click="savePromotionsDraft()" :disabled="promotions.length < 1" :loading="loading">
        <v-icon>
          mdi-content-save
        </v-icon>
        Guardar borrador
    ...
    <!-- PROMOTION IMAGE -->
    <v-file-input ref="promotionImage" class="d-none" accept=".jpg, .jpeg, .png"
      @update:model-value="getPromotionImage" />
  </v-row>
</template>
