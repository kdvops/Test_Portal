    ...
    <v-expand-transition>
      <v-card-text v-show="seoExpanded" class="pa-6">
        <v-row>
          ...
          <v-col cols="12" md="6">
            <v-text-field
              v-model="target.focusKeyword"
              label="Palabra Clave Principal"
              hint="Palabra clave más importante para este target"
              persistent-hint
            />
          </v-col>
          ...
          <v-col cols="12">
            <v-textarea
              v-model="target.metaDescription"
          ...
          <v-col cols="12">
            <v-textarea
              v-model="target.socialDescription"
          ...
          <v-col cols="12" v-if="target.keywords && target.keywords.length > 0">
            <div class="d-flex flex-wrap gap-2">
              <v-chip
                v-for="keyword in target.keywords"
                :key="keyword"
                closable
                color="#12539b"
                variant="outlined"
          ...
          <v-col cols="12" v-if="target.tags && target.tags.length > 0">
            <div class="d-flex flex-wrap gap-2">
              <v-chip
          ...
          <v-col cols="12" md="6">
            <v-card class="pa-4 text-center" @click="$refs.twitterImageUploader.click()" style="cursor: pointer">
              ...
              <v-img
                v-if="target.twitterImage"
                :src="target.twitterImage"
                height="100"
                class="mt-2"
                cover
              />
          ...
          <v-col cols="12" md="6">
            <v-select
              v-model="target.robotsDirectives"
          ...
          <v-col cols="12">
            <v-switch
              v-model="target.isFeatured"
              label="Target Destacado"
              color="#12539b"
              inset
            />
          </v-col>
...
@NuxtComponent({
  name: "seo-targets-component",
})
class SeoTargetsComponent extends Vue {
  // PROPS
  @Prop({ required: true }) target!: TargetInterface;
    ...
    if (file) {
      if (file.type === "image/png" || file.type === "image/jpeg") {
        if (file.size <= 2000000) {
          ...
          fr.onload = (el: any) => {
            this.target.ogImage = el.target.result;
          };
    ...
    if (file) {
      if (file.type === "image/png" || file.type === "image/jpeg") {
        if (file.size <= 2000000) {
          const fr = new FileReader();
          fr.onload = (el: any) => {
            this.target.twitterImage = el.target.result;
          };
          fr.readAsDataURL(file);
        } else {
          this.$bus.$emit("handleError", "El peso máximo es de 2 MB");
...
export default SeoTargetsComponent;
</script>
