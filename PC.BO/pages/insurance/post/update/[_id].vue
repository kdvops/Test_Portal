...
// IMPORT MUTATIONS
import { UPDATE_INSURANCE } from "~/graphql/mutations/insurance.mutation";
...
class InsuranceUpdateScreen extends Vue {
  ///////////////
  ///// REF /////
  ///////////////

  // CATEGORY REF
  @Ref('bannerInsuranceImage') bannerInsuranceImage!: any;
  ...
  // GET PARAM INSURANCE ID
  public insuranceID = useRoute().params._id;
  ...
  public insurance: InsuranceInterface = {
    title: '',
    excerpt: '',
    link: '',
    slug: '',
  ...
  public selectImage(targetImage: string) {
    ...
    // GET REFERENCE IMAGE
    const imageRefs: any = this.bannerInsuranceImage
  ...
  public getInsuranceImage(file: File) {
    if (file) {
  ...
  // SET INSURANCE BY ID
  public async setInsuranceById() {
    try {

      // PAYLOAD BY ID
      const insuranceIdDto = {
        insuranceId: this.insuranceID
      }

      // GET INSURANCE BY ID
      const { data } = await this.$apollo.query({
        query: GET_INSURANCE_BY_ID,
        variables: insuranceIdDto,
        fetchPolicy: 'no-cache'
      })

      // SET INSURANCE
      const insurance = data.findInsuranceById;

      // DECRYPT BASE64 TEXT TO HTML
      const sections = insurance.sections.map((section: any) => {
        return {
          ...section,
      ...
      // SET INSURANCE TO VARIABLE
      this.insurance = { ...insurance, sections };
  ...
  public async updateInsurance() {
    ...
    try {
      ...
      const insuranceClean = _.omit(this.insurance, [
        '_id',
        '__typename',
        'sections.__typename',
      ...
      const sectionCleanInsuranceClean = insuranceClean.sections?.map((section: any) => {
      ...
      // INSURANCE FINAL
      const insuranceFinal = {
        ...insuranceClean,
        status: 'publish',
        category: typeof this.insurance.category === 'string' ? this.insurance.category : this.insurance.category._id,
        sections: sectionCleanInsuranceClean
      }

      // PAYLOAD UPDATE INSURANCE
      ...
      // GO TO LIST
      this.$router.push('/insurance/post/list')
  ...
  // UPDATE INSURANCE DRAFT
  public async updateInsuranceDraft() {
    // SET LOADING
    this.loading = true;
  ...
  public get validUpdatePost(): boolean {
    return this.insurance.title === '' ||
      !this.insurance.category ||
      this.insurance.sections.length === 0
  }
  ...
  public newSection(section: SectionTypeInterface) {
    this.insurance.sections = [
      ...this.insurance.sections,
      {
        ...section,
        _id: section._id ?? this.makeid(8),
        position: this.insurance.sections.length + 1
      }
    ];
  }
  // EMIT EVENT EDIT SECTION
  public editSection(section: SectionTypeInterface) {
    this.insurance.sections[section.position - 1] = section;
  }
  ...
  public changePositionSection(evt: any) {
    ...
    // MOVE SECTION
    newSections = [...this.insurance.sections]
    ...
    // REASSIGN INSURANCE TO TRIGGER REACTIVITY
    this.insurance.sections = newSections;
  }

  // EMIT EVENT REMOVE SECTION
  public removeSection(position: number) {
    this.insurance.sections.splice(position - 1, 1);
    // REFRESH POSITION
    this.insurance.sections = this.insurance.sections.map((section, index) => {
      return {
        ...section,
        position: index + 1
      }
  ...
  public createNewCardSection(section: { position: number, card: TypeCard }) {
    const getSection = this.insurance.sections[section.position - 1];
    getSection.cards = [
      ...getSection.cards,
  ...
  public updateSlug() {
    this.insurance.slug = toSlug(this.insurance.title.toLowerCase())
  }

  // SAVE ITEM
  public async saveInsuranceDraft() {
    // FIRST UPDATE PRODUCT STATUS DRAFT
    await this.updateInsuranceDraft();
...
export default InsuranceUpdateScreen;
</script>
...
<template>
  <v-row class="px-0 pt-10" justify="center" align-content="center">
    ...
    <v-col cols="3">
      <detailed-image-component color="#00a44f" rounded="xl" height="340" v-model="insurance.thumbnailImageDetail" :legacy-image="insurance.thumbnail" text="Cargar Miniatura"></detailed-image-component>
    </v-col>

    <v-col cols="7">
      <!-- BANNER LARGE IMAGE -->
      <detailed-image-component color="#00a44f" rounded="xl" height="160" v-model="insurance.bannerImageDetail" :legacy-image="insurance.banner" text="Cargar Banner" class="mt-2"></detailed-image-component>
      <!-- BANNER RESPONSIVE IMAGE -->
      <detailed-image-component color="#00a44f" rounded="xl" height="160" v-model="insurance.responsiveImageDetail" :legacy-image="insurance.responsive" text="Cargar Banner Responsive" class="mt-2"></detailed-image-component>
    </v-col>
    ...
    <v-col cols="10">
      <v-form v-model="valid" lazy-validation>
        <v-row justify="center">
          <v-col cols="4">
            <v-text-field v-model="insurance.title" prepend-inner-icon="mdi-text" density="compact" variant="solo"
              rounded="xl" label="Titulo" @change="updateSlug" />
          </v-col>
          <v-col cols="4">
            <v-text-field v-model="insurance.subtitle" prepend-inner-icon="mdi-text" density="compact" variant="solo"
              rounded="xl" label="Subtitulo" />
          </v-col>
          <v-col cols="4">
            <v-text-field v-model="insurance.slug" prepend-inner-icon="mdi-text" density="compact" variant="solo"
              rounded="xl" label="Slug" />
          </v-col>
          <v-col cols="6">
            <v-text-field v-model="insurance.link" prepend-inner-icon="mdi-text" density="compact" variant="solo"
              rounded="xl" label="Link" />
          </v-col>
          <v-col cols="6">
            <v-select v-model="insurance.category" prepend-inner-icon="mdi-apps" density="compact" variant="solo"
              rounded="xl" :items="categories" item-value="_id" item-title="name" label="Categoría" />
          </v-col>
          <v-col cols="12">
            <v-text-field v-model="insurance.excerpt" prepend-inner-icon="mdi-text" density="compact" variant="solo"
              rounded="xl" label="Extracto" />
          </v-col>
          <v-col cols="12">
            <v-textarea v-model="insurance.description" prepend-inner-icon="mdi-text" density="compact" variant="solo"
    ...
    <v-col cols="12" class="pb-15 mb-15">
      <section-component @removeCardSection="removeCardSection" @updateCardSection="updateCardSection"
        ...
        @changePositionSection="changePositionSection" @setDataTable="setDataTable" :sections="insurance.sections" />
    </v-col>

    <!-- NEW BUTTONS SAVE ITEMS -->
    <v-bottom-navigation bg-color="#12539b">
      <v-btn position="absolute" rounded="xl"
        :base-color="insurance.status && insurance.status === 'draft' ? 'orange' : 'green'"
        style="height: 30px; top: 15px; left: 10px" variant="tonal" readonly>
        {{ insurance.status && insurance.status === 'draft' ? 'Borrador' : 'Publicado' }}
      </v-btn>
      <v-btn @click="updateInsurance()" :loading="loading">
        <v-icon>
          mdi-upload
        </v-icon>
        Actualizar y publicar
      </v-btn>
      <v-btn @click="saveInsuranceDraft()" :loading="loading">
        <v-icon>
          mdi-content-save
        </v-icon>
        Guardar borrador
  ...
  <!-- UPLOAD IMAGE -->
  <v-file-input ref="bannerInsuranceImage" class="d-none" accept=".jpg, .jpeg, .png"
    @update:model-value="getInsuranceImage" />
</template>
