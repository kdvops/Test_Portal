...
// IMPORT MUTATIONS
import { CREATE_INSURANCE } from "~/graphql/mutations/insurance.mutation";
...
class InsuranceCreateScreen extends Vue {
  ///////////////
  ///// REF /////
  ///////////////
  ...
  public async createInsurance() {
    ...
    try {
      ...
      createInsuranceDto.createInsuranceDto.sections = this.insurance.sections.map((section) => {
  ...
  // CREATE INSURANCE DRAFT
  public async createInsuranceDraft() {
    // SET LOADING FALSE
    this.loading = true
  ...
  public get invalidImages(): boolean {
    return (
      !String(this.insurance.bannerImageDetail?.image ?? '').trim() || 
      !String(this.insurance.thumbnailImageDetail?.image ?? '').trim() || 
      !String(this.insurance.responsiveImageDetail?.image ?? '').trim()
    );
  }
  ...
  public async goPreview() {
    ...
    const post = await this.createInsuranceDraft();

    // GO EDIT AFTER CREATE POST
    this.$router.push(`/insurance/post/update/${post.createInsurance._id}`);

    // GO PREVIEW BEFORE CREATE PRODUCT
    const route = this.$router.resolve({
      path: '/previews/post',
      query: {
        postID: post.createInsurance._id,
        postType: 'insurance'
      }
    });
...
<template>
  <v-row class="px-0 pt-10" justify="center" align-content="center">
    ...
    <v-bottom-navigation bg-color="#12539b">
      <v-btn @click="createInsurance()" :disabled="validCreatePost || invalidImages" :loading="loading">
        <v-icon>
          mdi-upload
        </v-icon>
        Crear Post
      </v-btn>
      <v-btn @click="saveInsuranceDraft()" :disabled="validCreatePost" :loading="loading">
        <v-icon>
          mdi-content-save
        </v-icon>
        Guardar
