...
@NuxtComponent({
  name: "regulatory-categories-list-screen",
  components: {
...
class RegulatoryCategoriesListScreen extends Vue {
  ///////////////
  // VARIABLES //
  ///////////////
  ...
  public async setCategories() {
    try {
      await this.categoryStore.loadCategories("category::about-us");
  ...
  public goUpdate(categoryID: string) {
    this.$router.push(`/regulatory/categories/update/${categoryID}`);
  }

  // GO TO CREATE SLIDER
  public goCreate() {
    this.$router.push(`/regulatory/categories/create`);
  }

  // GET CATEGORIES FOR TARGET
  public get categoriesForTarget(): Array<CategoriesInterface> {
    const categories = this.categories;
    const categoriesRegulatory = categories.filter(
      (category: CategoriesInterface) =>
        category.target === "categories::about-us"
    );

    return categoriesRegulatory;
  }
...
export default RegulatoryCategoriesListScreen;
</script>
          ...
          <v-card-text>
            <v-row align-content="center" justify="space-between">
              ...
              >
                ...
                >
                  Lista de categorías de archivos,
                </p>
                <p class="text-caption text-white font-weight-light ml-10">
                  Ahora puedes gestionar tus categorías de archivos regulatorios
