...
// IMPORT MUTATIONS
import { CREATE_FINANCIALLY } from "~/graphql/mutations/financially.mutation";
...
import {
  ...
  mapToCreateGridDto,
  mapToCreateGridsDto,
} from "~/interfaceMap/dtoMapping"; // INIT COMPONENTS CLASS
...
@NuxtComponent({
  name: "financially-create-screen",
  components: {
...
class FinanciallyCreateScreen extends Vue {
  ///////////////
  ///// REF /////
  ///////////////
  ...
  // CREATE FINANCIALLY CARD
  public async createFinancially() {
    // SET LOADING FALSE
    this.loading = true;

    try {
      // PAYLOAD PARENT AND TARGET DTO
      const createFinanciallyDto = {
        createFinanciallyDto: {
          ...
          file: this.newUploadFile.length > 0 ? this.newUploadFile : [],
        },
      };

      // ENCRYPT HTML TO BASE64
      createFinanciallyDto.createFinanciallyDto.sections =
        this.financially.sections.map((section) => {
      ...
      await this.$apollo.mutate({
        mutation: CREATE_FINANCIALLY,
        variables: createFinanciallyDto,
  ...
  // CREATE FINANCIALLY DRAFT
  public async createFinanciallyDraft() {
    // SET LOADING FALSE
    this.loading = true;
  ...
  public get validCreatePost(): boolean {
    return (
      this.financially.title === "" ||
      this.financially.excerpt === "" ||
      this.financially.subtitle === "" ||
      this.financially.description === "" ||
      !String(this.financially.bannerImageDetail?.image ?? "").trim() ||
      !String(this.financially.thumbnailImageDetail?.image ?? "").trim() ||
      !String(this.financially.responsiveImageDetail?.image ?? "").trim()
    );
  }
  ...
  public async goPreview() {
    ...
    const post = await this.createFinanciallyDraft();

    // GO EDIT AFTER CREATE POST
    this.$router.push(`/financially/update/${post.createFinancially._id}`);

    // GO PREVIEW BEFORE CREATE PRODUCT
    const route = this.$router.resolve({
      path: `/previews/financially/${post.createFinancially._id}`,
    });
...
export default FinanciallyCreateScreen;
</script>
...
<template>
  <v-row class="px-0 pt-10" justify="center" align-content="center">
    ...
    <v-col cols="10">
      <v-form v-model="valid" lazy-validation>
        <v-row justify="center">
          ...
          <v-col v-if="financially.type === 'postRelease'" cols="8">
            <v-hover>
              <template v-slot:default="{ isHovering, props }">
                <v-sheet
                  :color="newUploadFile.length > 0 ? 'green' : '#12539b'"
                  rounded="xl"
                  height="180"
                  v-bind="props"
                >
                  <!-- LAYER UPLOAD IMAGE -->
                  <v-container
                    v-if="isHovering || newUploadFile.length === 0"
    ...
    <v-bottom-navigation bg-color="#12539b">
      ...
      <template v-if="financially.type !== 'postRelease'">
        <v-btn
          @click="goPreview()"
          :disabled="validCreatePost"
          :loading="loading"
