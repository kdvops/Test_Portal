...
@Injectable()
export class SlugFormsInterceptor implements NestInterceptor {
  constructor(
    @InjectModel(Forms.name)
    private readonly model: Model<Forms & Document>,
  ) {}
  ...
  private async processItem(item: any): Promise<any> {
    ...
    if (
      ...
      console.log(doc);
      const baseSlug = slugify(`formulario-${String(doc._id).substring(10)}`, {
        lower: true,
        strict: true,
      });
