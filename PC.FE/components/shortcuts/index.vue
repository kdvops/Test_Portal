...
export default class AppShortcutsComponent extends Vue {
  ///////////////
  //// PROPS ////
  ///////////////
  ...
  public get cardsSelected() {
    ...
    const selectedShortcut = this.shortcuts.find((shortcut) => shortcut._id === this.shortcutSelected);
    return selectedShortcut?.cards || [];
  }
...
<template>
  <v-container class="pa-0 pa-sm-10">
    <template v-if="type === 'tabs'">
        ...
        <v-col class="py-0" cols="12" md="8" lg="8" xl="6">
          ...
          >
            <v-slide-group class="text-center justify-center h-100" show-arrows>
              <template v-for="shortcut in shortcuts" :key="`shortcut-${shortcut._id}`">
                <v-slide-group-item
                  v-if="shortcut && shortcut.type === 'shortcutTabs'"
      ...
      <v-row v-if="cardsSelected && cardsSelected.length > 0" class="mt-10">
        <template v-for="(card, index) in cardsSelected" :key="`card-${card._id}-${index}`">
          <v-col
            v-if="card && card.style === 'cardsShortcutSmall'"
            class="mx-auto"
            cols="12"
            md="10"
            lg="4"
          >
            <v-card
              ...
              class="mx-2 my-2 shortcut-card-small"
              rounded="lg"
              :color="card.background"
              :dark="card.background === '#000000'"
            >
              <CardImage 
                v-if="card.picture || card.pictureImageDetail?.image"
                :src="card.picture ?? card.pictureImageDetail?.image"
                :alt="card.pictureImageDetail?.altText ?? ''"
                :width="400" 
                :height="180"
                container-class="shortcut-card-image-container"
              />
              <div class="shortcut-card-overlay">
                <p
          ...
          <v-col
            v-if="card && card.style === 'cardsShortcutMedium'"
            class="mx-auto"
            cols="12"
            md="12"
            lg="6"
          >
            <v-card
              height="320"
              class="mx-2 my-2 shortcut-card-medium"
              rounded="lg"
              :color="card.background"
              :dark="card.background === '#000000'"
            >
          ...
          <v-col
            v-if="card && card.style === 'cardsShortcutLarge'"
            cols="12"
          >
            <v-card
              class="mx-2 my-2 shortcut-card-large"
              height="300"
              rounded="lg"
              :color="card.background"
              :dark="card.background === '#000000'"
    ...
    <template v-else-if="type === 'tabs-redirect'">
      <v-row justify="center">
        <v-col class="py-0" cols="12" md="8" lg="8" xl="6">
          ...
          >
            <v-slide-group class="text-center justify-center h-100" show-arrows>
              <template v-for="shortcut in shortcuts" :key="`shortcut-redirect-${shortcut._id}`">
                <v-slide-group-item 
                  v-if="shortcut"
                  style="position: relative" 
                  class="h-100"
...
.shortcut-card-small,
.shortcut-card-medium,
.shortcut-card-large {
  position: relative;
  overflow: hidden;
}

.shortcut-card-image-container {
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
...
.shortcut-card-overlay p,
.shortcut-card-overlay .v-btn,
.shortcut-card-overlay .v-icon {
  pointer-events: auto;
}
