...
// IMPORT MUTATIONS
import { CREATE_PROUSER_POST } from "~/graphql/mutations/prouser.mutation";
...
class ProuserCreateScreen extends Vue {
  ///////////////
  ///// REF /////
  ///////////////
  ...
  public async createProuser() {
    ...
    try {
      ...
      createProuserDto.createProuserDto.sections = this.prouser.sections.map((section) => {
      ...
      await this.$apollo.mutate({
        mutation: CREATE_PROUSER_POST,
        variables: createProuserDto,
        fetchPolicy: 'no-cache'
      })
  ...
  // CREATE PROUSER DRAFT
  public async createProuserDraft() {
    // SET LOADING FALSE
    this.loading = true
  ...
  public get invalidImages(): boolean {
    return (
      !String(this.prouser.bannerImageDetail?.image ?? '').trim() || 
      !String(this.prouser.thumbnailImageDetail?.image ?? '').trim() || 
      !String(this.prouser.responsiveImageDetail?.image ?? '').trim()
    );
  }
  ...
  public updateSlug(){
   ...
   public async goPreview() {
    ...
    const post = await this.createProuserDraft();

    // GO EDIT AFTER CREATE POST
    this.$router.push(`/prouser/post/update/${post.createProuser._id}`);

    // GO PREVIEW BEFORE CREATE PRODUCT
    const route = this.$router.resolve({ 
      path: '/previews/post',
      query: {
        postID: post.createProuser._id, 
        postType: 'prouser'
      } 
    });
...
export default ProuserCreateScreen;
</script>
  ...
  <v-row class="px-0 pt-10" justify="center" align-content="center">
    ...
    <v-col cols="12" class="pb-15 mb-15">
     ...
     <v-bottom-navigation bg-color="#12539b">
      <v-btn @click="createProuser()" :disabled="validCreatePost || invalidImages" :loading="loading">
        <v-icon>
          mdi-upload
        </v-icon>
        Crear Post
      </v-btn>
      <v-btn @click="saveProuserDraft()" :disabled="validCreatePost" :loading="loading">
        <v-icon>
          mdi-content-save
        </v-icon>
        Guardar
